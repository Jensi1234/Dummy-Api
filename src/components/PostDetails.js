...
const PostDetails = () => {
  ...
  const { loading: isLoading, error, data: post } = useFetchData(getPostById, pid);
  const userid = post?.userId;
  const { data: comments, setData: setComment } = useFetchData(getCommentsPostById, pid);
  ...
  const submitComment = async (commentId) => {
    const updateComment = await updatedCommentById(commentId, editComment)
